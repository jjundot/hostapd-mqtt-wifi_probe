...
{
	if(os_strcmp(buf, "use_mqtt") == 0)
	{
		os_strlcpy(USE_MQTT, pos,
			sizeof(USE_MQTT));
		printf("USE_MQTT %s\r\n",USE_MQTT);
	}
	else if(os_strcmp(buf, "mqtt_broker") == 0)
	{
		os_strlcpy(MQTT_BROKER, pos,
			sizeof(MQTT_BROKER));
	}
	else if(os_strcmp(buf, "mqtt_port") == 0)
	{
		os_strlcpy(MQTT_PORT, pos,
			sizeof(MQTT_PORT));
	}
	else if(os_strcmp(buf, "mqtt_username") == 0)
	{
			os_strlcpy(MQTT_USERNAME, pos,
		sizeof(MQTT_USERNAME));
	}
	else if(os_strcmp(buf, "mqtt_password") == 0)
	{
		os_strlcpy(MQTT_PASSWORD, pos,
